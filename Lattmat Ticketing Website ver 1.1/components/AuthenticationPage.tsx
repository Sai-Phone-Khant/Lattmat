...
export function AuthenticationPage({ onClose, onSuccess }: AuthenticationPageProps) {
  ...
  const [googleUserData, setGoogleUserData] = useState<{ email: string } | null>(null);
  const [isLoading, setIsLoading] = useState(true); // Start with loading true to check session
  const [nameInputValue, setNameInputValue] = useState('');

  // Use singleton Supabase client from context
  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        console.log('Checking for authentication session...');
        ...
        if (isOAuthCallback) {
          ...
          if (data.session?.user) {
            console.log('OAuth successful, user session found:', data.session.user);
            ...
            if (userName) {
              ...
              console.log('User has name, completing authentication');
              onSuccess({
                name: userName,
                email: userEmail
              });
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        } else if (userEmail && !showNameInput) {
          setGoogleUserData({ email: userEmail });
          setShowNameInput(true);
          setIsLoading(false);
        }
  ...
  }, [onSuccess, showNameInput, supabase.auth]);
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      // Use Supabase's Google OAuth with dynamic redirect
      ...
      // The useEffect will handle the callback when user returns
      
    } catch (error) {
      console.error('Google auth error:', error);
      alert(`Google authentication failed: ${error instanceof Error ? error.message : 'Please try again.'}`);
    }
  };
  ...
  const handleNameSubmit = async () => {
    ...
    try {
      ...
      alert(`Failed to save name: ${error instanceof Error ? error.message : 'Please try again.'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoading && !showNameInput) {
    return (
      <div className="min-h-screen relative flex items-center justify-center px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-md w-full relative z-10 p-8">
          <div className="text-center">
            <div className="w-8 h-8 border-2 border-blue-700 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            ...
            {showNameInput ? (
              ...
              <div>
                ...
                <div className="text-center mb-8">
                  ...
                  <h2 className="text-blue-700 text-xl font-medium mb-2">Welcome to Lattmat!</h2>
              ...
              <div>
                ...
                <div className="text-center mb-8">
                  ...
                  <h2 className="text-blue-700 text-xl font-medium mb-2">Lattmat</h2>
                  <p className="text-blue-700 text-lg">Don't miss your happiness</p>
                </div>
                ...
                <button
                  ...
                  disabled={isLoading || (!isSignUp && (!formData.phone || !formData.password)) || (isSignUp && (!formData.name || !formData.phone || !formData.password))}
                ...
                >
                  ...
                  <span className="text-gray-700">{isSignUp ? 'Sign up' : 'Sign in'} with Gmail</span>
                </button>
              </div>
            )}
          </div>
