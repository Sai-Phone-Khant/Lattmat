...
export function PaymentPage({ event, ticketData, onBack, onHome }: PaymentPageProps) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Prepare transaction data for the new database schema
      const deliveryOption = formData.ticketOption === 'Physical ticket' ? 
        (physicalTicketOption === 'delivery' ? 'Home Delivery' : 'Pick Up from Partner Store') : 
        'E-ticket';
      ...
      const transactionData = {
        ...
        billing_address: formData.ticketOption === 'Physical ticket' && physicalTicketOption === 'delivery' ? 
          formData.address : 
          'No address required for E-ticket/Pickup option', // billing_address is required in schema
        ticket_delivery_option: deliveryOption,
        payment_method: paymentMethod,
        selected_tickets: ticketData.selectedTickets,
        event_group_id: ticketData.selectedEventGroup?.id,
        total_amount: total,
        user_id: null // Set to null for guest purchases, could be populated for authenticated users
      };

      console.log('Saving ticket transaction:', transactionData);

      // Save to database
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-ecf3a44f/ticket-transactions`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        console.error('Failed to save ticket transaction:', result);
        alert('Failed to process order. Please try again.');
        return;
      }

      console.log('Ticket transaction saved successfully:', result);

      // Create order data for UI
      const now = new Date();
      const newOrderData = {
        orderId: orderId,
